...
const VerifyOtp = () => {
  const { verifyOtpClient, user } = useGlobalContext();
  const navigate = useNavigate();
  const [otp, setOtp] = useState();
  const handleSubmit = (e) => {
    e.preventDefault();
    const formData = { userId: user?._id, otp };
    console.log(formData);
    verifyOtpClient(formData, navigate);
  };
  useEffect(() => {
    if (!user) {
      navigate("/login");
          ...
          <div class="mx-auto flex w-full max-w-md flex-col space-y-16">
            <div class="flex flex-col items-center justify-center text-center space-y-2">
              ...
              <div class="flex flex-row text-sm font-medium text-gray-400">
                <p>We have sent a code to your phone {user?.phone}</p>
              </div>
            </div>
            <div>
              <form action="" method="post" onSubmit={handleSubmit}>
                <div class="flex flex-col space-y-16">
                  ...
                  <div class="flex flex-col space-y-5">
                    <div>
                      <button class="flex flex-row items-center justify-center text-center w-full border rounded-xl outline-none py-5 bg-yellow-400 border-none text-white text-sm shadow-sm">
                        Verify Account
                      </button>
                    </div>
